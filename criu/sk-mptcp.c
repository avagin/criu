...
static int dump_mptcp_conn_state(struct inet_sk_desc *sk)
{
	...
	if (data.snd_data_fin_enable)
		mse.seq_flags |= MPTCP_SEQ_FLAG_SND_DATA_FIN;
	if (data.allow_infinite_fallback)
		mse.seq_flags |= MPTCP_SEQ_FLAG_INFINITE_FB;
...
static int restore_mptcp_conn_state(int sk, struct libsoccr_sk *socr, struct inet_sk_info *ii)
{
	...
	data.snd_data_fin_enable = !!(mse->seq_flags & MPTCP_SEQ_FLAG_SND_DATA_FIN);
	data.allow_infinite_fallback = !!(mse->seq_flags & MPTCP_SEQ_FLAG_INFINITE_FB);
