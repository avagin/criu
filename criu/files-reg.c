				...
				unsigned char **build_id)
{
	...
	program_header = (Elf32_Phdr *) (file_header->e_phoff + (uintptr_t) file_header);
	...
	note_header = (Elf32_Nhdr *) (program_header->p_offset + (uintptr_t) file_header);
	...
	while (num_iterations-- && note_header <= note_header_end &&
		...
		note_header = (Elf32_Nhdr *) ((uintptr_t) note_header + sizeof(Elf32_Nhdr) +
	...
	note_header = (Elf32_Nhdr *) ((uintptr_t) note_header + sizeof(Elf32_Nhdr) + note_header->n_namesz);
				...
				unsigned char **build_id)
{
	...
	program_header = (Elf64_Phdr *) (file_header->e_phoff + (uintptr_t) file_header);
	...
	note_header = (Elf64_Nhdr *) (program_header->p_offset + (uintptr_t) file_header);
	...
	while (num_iterations-- && note_header <= note_header_end &&
			note_header->n_type != NT_GNU_BUILD_ID) {
		note_header = (Elf64_Nhdr *) ((uintptr_t) note_header + sizeof(Elf64_Nhdr) +
						note_header->n_namesz + note_header->n_descsz);
	}
	if (!num_iterations || note_header >= note_header_end) {
		munmap(file_header, fd_status->st_size);
	...
	size = note_header->n_descsz;
	note_header = (Elf64_Nhdr *) ((uintptr_t) note_header + sizeof(Elf64_Nhdr) + note_header->n_namesz);
