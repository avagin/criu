...
#define ASYNC_THREAD_NR 16 /* FIXME */
static int asyncd(int sk)
{
	pthread_t threads[ASYNC_THREAD_NR];
	int i;

	for (i = 0; i < ASYNC_THREAD_NR; i++) {
...
int stop_asyncd(void)
{
	...
	if (status != 0)
		pr_err("async: daemon exited abnormally\n");
	else
