...
static int do_open_fifo(int ns_root_fd, struct reg_file_info *rfi, void *arg)
{
	...
	/*
	 * An end which had not seen a writer has to be reopened with no
	 * writer around too, or it comes back as a closed pipe and the
	 * application polling it spins. A fifo with buffered data is left
	 * alone, as the fake writer is the one holding the data.
	 */
	bool drop_writer_early = read_only && nonblocking && pipe_empty && wait_writer;
	...
	/*
	 * Closing the last descriptor destroys the pipe object together
	 * with its restored parameters, so when the writer is dropped
	 * early the pipe parameters are restored on the final descriptor
	 * below instead.
	 */
	if (drop_writer_early)
