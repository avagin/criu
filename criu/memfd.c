			    ...
			    const char *name, const struct stat *st)
{
	...
	u32 shmid;
	 ...
	 /*
	  * shmids are the inode number of the mmaped file.
	  * See handle_vma() in proc_parse.c.
	  * It works for memfd, because we share the same device
	  * as the shmem device.
	  */
	shmid = inode->ino;

	pr_info("Dumping memfd:%s contents (id %#x, shmid: %#x, size: %ld)\n",
...
int memfd_open(struct file_desc *d, u32 *fdflags)
{
	...
	/* Reopen the fd with one with proper permissions */
