...
int check_options(void)
{
	...
	if (opts.compress_mode == COMPRESS_REGION && opts.compress_region_size > PAGE_SIZE) {
		if (opts.use_page_server || opts.addr) {
			pr_err("Multi-page --compress-region is not supported with --page-server (use region size %lu)\n",
			       (unsigned long)PAGE_SIZE);
			return 1;
		}
		if (opts.stream) {
			pr_err("Multi-page --compress-region is not supported with --stream (use region size %lu)\n",
			       (unsigned long)PAGE_SIZE);
			return 1;
		}
	}
