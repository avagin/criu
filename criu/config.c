...
static int parse_file_validation_method(struct cr_options *opts, const char *optarg)
{
	...
	} else if (!strcmp(optarg, "chksm")) {
		opts->file_validation_method = FILE_VALIDATION_CHKSM;
		opts->file_validation_chksm_config = FILE_VALIDATION_CHKSM_EVERY;
	} else if (!strcmp(optarg, "chksm-first")) {
		opts->file_validation_method = FILE_VALIDATION_CHKSM;
		opts->file_validation_chksm_config = FILE_VALIDATION_CHKSM_FIRST;
	} else if (!strcmp(optarg, "chksm-period")) {
		opts->file_validation_method = FILE_VALIDATION_CHKSM;
...
#define BOOL_OPT(OPT_NAME, SAVE_TO) \
	...
	static struct option long_opts[] = {
		...
		{ "chksm-parameter",		required_argument,	0, 1099 },
		{ },
	};
