...
static char *image_name(struct cr_img *img, char *image_path, size_t image_path_size)
{
	int fd = img->_x.fd;
...
int do_pb_read_one(struct cr_img *img, void **pobj, int type, bool eof)
{
	...
	if (!cr_pb_descs[type].pb_desc) {
		pr_err("Wrong object requested %d on %s\n",
			type, image_name(img, imgp, sizeof(imgp)));
		return -1;
	}
	...
	*pobj = NULL;
	...
	if (ret == 0) {
		if (eof) {
			...
			pr_err("Unexpected EOF on %s\n",
			       image_name(img, imgp, sizeof(imgp)));
			return -1;
		}
	...
	if (ret < 0) {
		pr_perror("Can't read %d bytes from file %s",
			  size, image_name(img, imgp, sizeof(imgp)));
		goto err;
	} else if (ret != size) {
		pr_perror("Read %d bytes while %d expected from %s",
			  ret, size, image_name(img, imgp, sizeof(imgp)));
		ret = -1;
		goto err;
	}
	...
	*pobj = cr_pb_descs[type].unpack(NULL, size, buf);
	if (!*pobj) {
		...
		pr_err("Failed unpacking object %p from %s\n",
		       pobj, image_name(img, imgp, sizeof(imgp)));
		goto err;
	}
