			   ...
			   const char *path)
{
	...
	if (is_memfd(vfi_dev, fname)) {
		...
		/* The error from previous open(), EPERM, will be shown */
			...
			int *vm_file_fd)
{
	...
	if (vma_area->file_borrowed) {
		 ...
		 *
		 * We treat memfd mappings as normal files mmaps because their
		 * backing files can be seen as file descriptors, which we
		 * dump as ghost files. So even though memfd is an anonymous
		 * shmem, we treat it differently.
		 */
