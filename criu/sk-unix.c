	...
	*fown = p->fown;
	...
	if (ue->peer) {
		...
		if (sk->wqlen != 0) {
			...
			pr_err("Non-empty write queue on an in-flight socket "
			       "%#x\n",
			       ue->ino);
			goto err;
		}
		...
		for (i = 0; i < sk->nr_icons; i++)
			if (sk->icons[i] == 0) {
				// Inode of an icon socket equal to 0 means it's
				// already been closed. That means we have no
				// simple way to check if it sent any data.
				// The only good solution for now is to fail the
				// migration.
				pr_err("Found a closed in-flight socket to "
				       "%#x\n",
				       ue->ino);
				goto err;
			}
