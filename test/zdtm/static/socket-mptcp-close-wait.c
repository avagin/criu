...
const char *test_doc = "Check MPTCP sockets in TCP_CLOSE_WAIT and TCP_LAST_ACK states\n";
const char *test_author = "Andrey Vagin <avagin@google.com>";
...
int main(int argc, char **argv)
{
	...
	if (extpid < 0) {
		...
		fd = mptcp_init_client(ZDTM_FAMILY, "localhost", port);
		if (fd < 0)
			return 1;

		ctl_fd = tcp_init_client(ZDTM_FAMILY, "localhost", port);
		if (ctl_fd < 0)
			return 1;
	...
	if ((fd_s = mptcp_init_server(ZDTM_SRV_FAMILY, &port)) < 0) {
	...
	/*
	 * parent is server of MPTCP connection
	 */
