...
uint64_t calc_sym_offset(const char *sym_name)
{
	...
	if (!elf) {
		pr_err("Failed to read self-executable\n");
		goto out_fd;
	}
...
struct uprobe_context enable_uprobe(const char *file, uint64_t offset)
{
	...
	if (!trace_instance) {
		pr_err("Failed to create tracefs instance\n");
		return context;
	}
