...
static pid_t start_target(const char *trigger, bool threaded, bool exit_thread, pid_t *tid, int *target_sock)
{
	int ready[2];
	int rpc_sock[2];
	pid_t pid;
	...
	assert(pipe(ready) == 0);
	assert(socketpair(AF_UNIX, SOCK_SEQPACKET, 0, rpc_sock) == 0);
	pid = fork();
	assert(pid >= 0);
	if (!pid) {
...
static pid_t check_api_calls(const char *path, bool completed, bool driver, bool restore_fault, bool helper_error)
{
	...
	while (fscanf(file, "%31s %d %d", operation, &target_pid, &caller) == 3) {
		int status;
		bool in_worker = driver &&
				 (!strcmp(operation, "checkpoint") || !strcmp(operation, "init") ||
				  !strcmp(operation, "restore") || !strcmp(operation, "unlock"));
		...
		} else if (strcmp(operation, "get-state") != 0) {
			assert(caller == getpid());
		}
		     ...
		     const struct cuda_plugin_backend *backend)
{
	...
	assert(unsetenv("CRIU_CUDA_MOCK_TARGET_FD") == 0);
	...
	assert(log_file);
	pid = start_target(trigger, driver, !strcmp(behavior, "target-exit"), &tid, &target_sock);
	target_pid = pid;
	if (driver) {
		assert(pid != tid);
		snprintf(value, sizeof(value), "%d", tid);
		assert(setenv("CRIU_CUDA_MOCK_RESTORE_TID", value, 1) == 0);
		snprintf(value, sizeof(value), "%d", target_sock);
		assert(setenv("CRIU_CUDA_MOCK_TARGET_FD", value, 1) == 0);
	}
	...
	if (stop_timeout) {
		...
		assert(elapsed >= 10);
		if (!driver) {
			/* The CLI backend cannot restore ptrace settings without a stop. Finish
	...
	} else if (completed || (!driver && !target_dead)) {
	...
	if ((!driver || completed) && strcmp(behavior, "target-kill"))
...
int main(int argc, char **argv)
{
	...
	const char *driver_cases[] = { "success", "api-error", "fault", "trap",
