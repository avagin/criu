...
static int get_cuda_restore_pid(int root_pid)
{
	char pid_buf[16];
...
static int interrupt_restore_thread(int restore_pid)
{
	 ...
	 */
	if (ptrace(PTRACE_INTERRUPT, restore_pid, NULL, 0)) {
		pr_err("Could not interrupt cuda restore tid %d after checkpoint, process may be in strange state\n",
		       restore_pid);
		return -1;
	}

	/* compel_wait_task() will fail here as expected but the work we need it to
	 * do is actually done before the sigstop check that fails. Without this
...
int cuda_plugin_checkpoint_devices(int pid)
{
	...
	restore_pid = get_cuda_restore_pid(pid);
	...
	pr_info("Checkpointing CUDA devices on pid %d restore_pid %d\n", pid, restore_pid);
	/* We need to resume the resume the checkpoint thread to prepare the
	 * mappings for checkpointing
	 */
	if (ptrace(PTRACE_CONT, restore_pid, NULL, 0)) {
		pr_err("Could not resume cuda restore pid %d for checkpoint on pid %d\n", restore_pid, pid);
		return -1;
	}
	int status = cuda_process_checkpoint_action(pid, ACTION_CHECKPOINT, 0, msg_buf, sizeof(msg_buf));
	if (status) {
		pr_err("CHECKPOINT_DEVICES failed with %s\n", msg_buf);
		ret = -1;
		goto interrupt;
	}
	if (update_checkpointed_pid(&cuda_pids, pid)) {
...
interrupt:
	int_ret = interrupt_restore_thread(restore_pid);
...
int resume_device(int pid, int checkpointed)
{
	...
	int restore_pid = get_cuda_restore_pid(pid);
	if (restore_pid == -1) {
	 ...
	 */
	if (ptrace(PTRACE_CONT, restore_pid, NULL, 0)) {
		pr_err("Unable to resume cuda restore pid %d on root pid %d\n", restore_pid, pid);
		return -1;
	}
...
int cuda_plugin_init(int stage)
{
	if (!cuda_checkpoint_supports_flag("--action")) {
		pr_err("cuda-checkpoint --action flag not supported, an r555 or higher version driver is required. Disabling CUDA plugin\n");
